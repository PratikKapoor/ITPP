...
extern "C"
{
	...
	int sci_cpp_ofdm_mod(char *fname, unsigned long fname_len)
	{	
		...
		outputVector = OFDM((int)*inNfftMatrix,(int)*inNcpMatrix,(int)*inNupsampleMatrix).modulate(inputVector);
		
		/*=================================================================*/
	
		//Storing the output in a double array to be sent to scilab
		
		// Output Length = Nupsample*(input.length() / Nfft)*(Nfft + Ncp)
		
		int outLen = (((int)*inNupsampleMatrix)*inputVector.length()*((int)*inNfftMatrix+(int)*inNcpMatrix))/((int)*inNfftMatrix);
		
		//int outLen = inputVector.length()/((int)*inNfftMatrix);
